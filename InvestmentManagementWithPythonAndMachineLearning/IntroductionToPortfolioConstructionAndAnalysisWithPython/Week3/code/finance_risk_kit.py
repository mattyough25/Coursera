...
def get_hfi_returns(path):
    """
    Load and format the EDHEC Hedge Fund Index Returns
    """
    hfi = pd.read_csv(path,
                      header=0, index_col=0, parse_dates=True)
    hfi = hfi/100
    hfi.index = hfi.index.to_period('M')
    return hfi

def get_ind_file(path,filetype):
    """
    ...
    if filetype is "returns":
        ...
        name = "size"
        divisor = 1
    ind = pd.read_csv(f"{path}\\ind30_m_{name}.csv", header=0, index_col=0)/divisor
    ind.index = pd.to_datetime(ind.index, format="%Y%m").to_period('M')
    ind.columns = ind.columns.str.strip()
    return ind

def get_ind_returns(path):
    """
    Load and format the Ken French 30 Industry Portfolios Value Weighted Monthly Returns
    """
    return get_ind_file(path, "returns")

def get_ind_nfirms(path):
    """
    Load and format the Ken French 30 Industry Portfolios Average number of Firms
    """
    return get_ind_file(path,"nfirms")

def get_ind_size(path):
    """
    Load and format the Ken French 30 Industry Portfolios Average size (market cap)
    """
    return get_ind_file(path, "size")

                         
def get_total_market_index_returns(path):
    """
    Load the 30 industry portfolio data and derive the returns of a capweighted total market index
    """
    ind_nfirms = get_ind_nfirms(path)
    ind_size = get_ind_size(path)
    ind_return = get_ind_returns(path)
